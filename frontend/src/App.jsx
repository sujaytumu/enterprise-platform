...
export default function App() {
  ...
  async function handleCreateAccount(e) {
    ...
    try {
      ...
      setNewAccount({ holderName: '', openingBalance: '' });
      await refreshAll();
  ...
  async function handleIssueCard() {
    setError('');
    try {
      await api.issueCard(selectedAccountId);
  ...
  return (
    <div className="app">
      <header>
        ...
        <p className="subtitle">Reference demo — accounts, cards, transaction authorization</p>
      </header>
      ...
      <section className="panel">
        ...
        <div className="account-list">
          {accounts.map((a) => (
            <button
              key={a.id}
              className={`account-chip ${a.id === selectedAccountId ? 'selected' : ''}`}
      ...
      {selectedAccount && (
        <>
          ...
          <section className="panel">
            ...
            <table>
              <thead>
                <tr><th>Time</th><th>Merchant</th><th>Amount</th><th>Status</th></tr>
              </thead>
              <tbody>
                {transactions.map((t) => (
                  <tr key={t.id} className={t.status === 'DECLINED' ? 'declined' : ''}>
